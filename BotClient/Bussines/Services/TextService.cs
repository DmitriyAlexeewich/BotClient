...
namespace BotClient.Bussines.Services
{
    public class TextService : ITextService
    {
        ...
        private async Task<string> SetPhrases(string Text)
        {
            ...
            {
                settingsService.AddLog("TextService", ex);
            }
            return Text;
        }
        ...
        private async Task<string> ReplaceSecondaryText(string Text)
        {
            try
            {
                ...
                for (int i = 0; i < Text.Length; i++)
                {
                    ...
                    {
                        var substring = Text.Substring(lastOpen, i - lastOpen - 1);
                        var parts = substring.Substring(1, substring.Length).Split("_").ToList();
